...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
	...
	LoadStringW(hInstance, IDC_WINDOWSWORK62, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_WINDOWSWORK62));
...
ATOM MyRegisterClass(HINSTANCE hInstance) {
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WINDOWSWORK62));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_WINDOWSWORK62);
...
vector<vector<DropBlock*>> blocks;
int defaultSize = 25;

void initBlocks() {
	blocks.resize(BlockSizeY);
	for (size_t y = 0; y < BlockSizeY; y++) {
		blocks[y].resize(BlockSizeX);
...
COLORREF blockColor = RGB(255, 255, 255);
int color[3] = { 0, };
...
void renderLines(HDC hdc) {
	...
	if (color[0]|| color[1] || color[2] ) blockColor = RGB(0, 0, 0);
	if(color[0]) blockColor += RGB(255, 0, 0);
	if (color[1]) blockColor += RGB(0, 255, 0);
	if (color[2]) blockColor += RGB(0, 0, 255);

	HBRUSH b = CreateSolidBrush(blockColor);
	HBRUSH old = (HBRUSH)SelectObject(hdc, b);

	for (size_t i = 0; i < x; i++) {
		for (size_t j = 0; j < y; j++) {
			renderRect(hdc, i * s, j * s, s, s, blockColor);
		}
	}
...
int dropUpdateTime = 16;

void update(HWND hwnd) {
	GetClientRect(hwnd, &rectView);
	timer.update(dropUpdateTime);
